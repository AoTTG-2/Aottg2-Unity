...
|CanDamageLeftHand|bool|False|Can the left hand be damaged|
|CanDamageRightHand|bool|False|Can the right hand be damaged|
...
|HandSeverWindow|float|False|Time window in seconds for both hands to be severed before they recover independently.|
|LeftHandSeverTimeLeft|float|False|Time remaining in seconds for left hand to recover from severed state (0 if not severed).|
|RightHandSeverTimeLeft|float|False|Time remaining in seconds for right hand to recover from severed state (0 if not severed).|
|StunDuration|float|False|Duration in seconds that the colossal remains stunned after both hands are severed.|
|StunTimeLeft|float|False|Time remaining in seconds for the stun phase (0 if not stunned).|
|RecoveryDuration|float|False|Duration in seconds for the recovery phase after being stunned.|
|RecoveryTimeLeft|float|False|Time remaining in seconds for the recovery phase (0 if not recovering).|
|StunState|string|True|Colossal's current stun state. Refer to [StunStateEnum](../Enums/StunStateEnum.md)|
