...
{
    /// <summary>
    /// A data compressor for the <c>zlib</c> compression algorithm, utilizing the <see cref="System.IO.Compression.DeflateStream"/> class
    /// </summary>
    internal class DeflateAlgorithm : CompressionAlgorithm
    {
        public override byte[] Compress(byte[] data, CompressionLevel level = CompressionLevel.Fastest)
        {
            try
            {
                ...
                {
                    using (Stream compressor = new DeflateStream(memory, level, true))
                    {
        ...
        public override byte[] Decompress(byte[] data, int bufferSize = DefaultBufferSize, long maxSize = DefaultMaxSize)
        {
            try
            {
                using (MemoryStream memory = new MemoryStream())
                {
                    using (Stream decompressor = new DeflateStream(new MemoryStream(data), CompressionMode.Decompress))
                    {
