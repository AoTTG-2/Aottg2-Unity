...
{
    /// <summary>
    /// A data compressor for the <c>gzip</c> compression algorithm, utilizing the <see cref="System.IO.Compression.GZipStream"/> class
    /// </summary>
    internal class GZipAlgorithm : CompressionAlgorithm
    {
        public override byte[] Compress(byte[] data, CompressionLevel level = CompressionLevel.Fastest)
        {
            try
            {
                ...
                {
                    using (Stream compressor = new GZipStream(memory, level, true))
                    {
        ...
        public override byte[] Decompress(byte[] data, int bufferSize = DefaultBufferSize, long maxSize = DefaultMaxSize)
        {
            try
            {
                using (MemoryStream memory = new MemoryStream())
                {
                    using (Stream decompressor = new GZipStream(new MemoryStream(data), CompressionMode.Decompress))
                    {
